...
void MpmFile::write() {
    ...
    file << "spy " << min.Y() - down_extend.y << " " << max.Y() + up_extend.y
         << "\n";
    file << "spz " << min.Z() - down_extend.z << " " << max.Z() + up_extend.z
         << "\n";
    file << "dcell " << dx * dCell_scale << "\n";

    // Boundary Condition
    file << "\n";
    file << "fixed " << down_boundary.x << " " << up_boundary.x << " "
         << down_boundary.y << " " << up_boundary.y << " " << down_boundary.z
         << " " << up_boundary.z << "\n";

    // Detonation Point
    file << "\n";
    file << "deto " << detonation_point.x << " " << detonation_point.y << " "
         << detonation_point.z << "\n";
    ...
    if (Contact) {
        ...
        switch (contact_parameters.type) {
            case CONTACT::LAGRANGIAN:
                file << "lagr " << contact_parameters.friction << " "
